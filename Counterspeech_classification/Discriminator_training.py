import numpy as np
from Discriminator.model import *
from Discriminator.data import *
from Discriminator.eval import *
from Generation.utils import *
from transformers import AutoTokenizer, AutoModelForSequenceClassification,AutoModelForTokenClassification, AdamW, get_linear_schedule_with_warmup
...
#'task_name':'Counterspeech',
 

params={
 'model_path':'bert-base-uncased',
 'task_name':'Counterspeech',
 'save_path':'Saved_models/Discriminator/',
 'logging':'local',
...
def train(training_dataloader, validation_dataloader, test_dataloader, model, tokenizer, params,run,device):
    ...
    for epoch_i in tqdm(range(0, epochs)):
        ...
        for step, batch in tqdm(enumerate(training_dataloader), total=len(training_dataloader)):
            ...
            optimizer.zero_grad()
            results= model(input_ids=b_input_ids,attention_mask=b_input_mask,labels=b_labels,return_dict=True)
            loss=results.loss
...
def train_caller(params,run=None):
        ...
        dataset_path='Datasets/'+params['task_name']+'/'
        train_data,valid_data,test_data,class_label=load_data_own(data_path=dataset_path)
        print(class_label)
        params['num_classes']=len(class_label)
        if(params['task_name']=='Argument'):
            print("here")
            train_data_source = Normal_Dataset_Arg(train_data,class_label,tokenizer=tokenizer, params=params,train = True)
            val_data_source = Normal_Dataset_Arg(valid_data,class_label,tokenizer,params)
            test_data_source = Normal_Dataset_Arg(test_data,class_label,tokenizer, params)

        else:
            train_data_source = Normal_Dataset(train_data,class_label,tokenizer=tokenizer, params=params,train = True)
        ...
        model = AutoModelForSequenceClassification.from_pretrained(params['model_path'], cache_dir=params['cache_path'],output_attentions = True,output_hidden_states = False).to(device)
